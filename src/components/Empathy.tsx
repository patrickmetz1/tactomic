    ...
    <section className="py-10 bg-gradient-to-b from-white to-book-50/30" ref={sectionRef}>
      <div className="container mx-auto px-4">
        <div className="text-center mb-6 opacity-0" ref={headerRef}>
          <h2 className="heading-lg mb-2">Wearing too many hats?</h2>
          <p className="text-lg text-muted-foreground">Refocus on high leverage activities</p>
        </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-8">
            {painPoints.map((point, index) => (
              <Card 
                key={index} 
                className="opacity-0 hover:shadow-md transition-all duration-300 border border-gray-100 hover:border-book-200 hover:scale-[1.02] hover:bg-gradient-to-br from-white to-book-50/40"
                ref={el => {
                  if (el) {
                    const observer = new IntersectionObserver(entries => {
                      entries.forEach(entry => {
          ...
          <div 
            className="opacity-0 mt-4 mx-auto max-w-lg text-center" 
            ref={bubbleRef}
